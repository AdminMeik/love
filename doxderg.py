...
class DoxMod(loader.Module):
    """prank dox v1.0"""
    strings = {
        "name": "@YMEPOTBAC 05peek dox",
        "usage": "Загрузите модуль, чтобы использовать .derglist.",
    }
    ...
    @loader.command(alias='derg')
    async def dergcmd(self, message):
        """ - сделать фэйковый докс"""
        ...
        await utils.answer(message, "👁️ <b>Докс от 05peek запущен...</b>")
        await sleep(0.5)
        ...
        # Создаем более правдоподобные данные
        phone_code = "+7" if country == "Россия" else "+380" if country == "Украина" else "+375" if country == "Беларусь" else "+"
        formatted_phone = f"{phone_code}{phone_number[:3]} {phone_number[3:6]}-{phone_number[6:8]}-{phone_number[8:]}"
        
        # Генерация правдоподобного email на основе имени
        if possible_names:
        ...
        result_message = (
            ...
            f"🏥 Дата рождения: {self.get_random_value(birth_dates)}\n"
            f"📪 Email: {email}\n\n"
            ...
            f"\n<b>Докс сгенерирован системой 05peek</b>"
        )
        await utils.answer(message, f"{result_message}")

    @loader.command(alias="derglist")
    async def derglistcmd(self, message):
